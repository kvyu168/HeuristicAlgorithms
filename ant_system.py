...
class AntSystem:
    ...
    def initialize(self):
        ...
        self.heuristic_values = np.zeros((self.num_cities,self.num_cities))
        self.pheromone_map = np.zeros((self.num_cities,self.num_cities))
        ...
        for t in range(1,self.num_cities-1):
            ...
            for city_id in candidates:
                fitness = pow(self.pheromone_map[current_city_id][city_id],self.pheromone_factor)+\
                    pow(self.heuristic_values[current_city_id][city_id],self.heuristic_factor)
